...
#[inline]
fn br_to_ttf_vec(br: &[u8]) -> io::Result<Vec<u8>> {
...
#[inline]
fn insert_owned(defs: &mut FontDefinitions, key: &str, br: &[u8]) -> Result<(), BlpError> {
    let font_bytes = br_to_ttf_vec(br).map_err(|err| {
        BlpError::new("font-decompress")
            .with_arg("font", key.to_string())
            .push_std(err)
    })?;

    defs.font_data
        .insert(key.to_string(), Arc::from(FontData::from_owned(font_bytes)));
    Ok(())
}
...
pub fn install_fonts(ctx: &Context) {
    ...
    for (stem, br) in all_fonts_br() {
        if let Err(err) = insert_owned(&mut defs, stem, br) {
            eprintln!("Failed to register font {stem}: {err}");
        }
    }
