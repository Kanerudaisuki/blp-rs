use std::path::Path;
use normpath::PathExt; // cargo add normpath

pub fn abs_string_with_macros(p: &Path) -> String {
    // 1) абсолютный и очищенный от . / ..
    let abs = p.normalize().expect("normalize").into_path_buf();
        ...
        for var in ["USERPROFILE", "HOME"] {
            if let Ok(v) = std::env::var(var) {
                ...
                if has_prefix_boundary(&s, &pref, true) &&
                    best.as_ref().map_or(true, |(_, b)| pref.len() > b.len()) {
                    best = Some((var, pref));
                }
            }
        }
...
#[cfg(windows)]
fn trim_trailing_bslash(mut s: String) -> String { while s.ends_with('\\') && s.len()>1 { s.pop(); } s }
