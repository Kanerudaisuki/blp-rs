...
/// Find the start offset of the SOF0 segment and the end offset of the SOS header (first entropy byte).
pub fn find_sof0_and_sos_end(jpeg: &[u8]) -> Result<(usize, usize), BlpError> {
    if jpeg.len() < 4 {
        return Err(BlpError::new("jpeg_too_short"));
    }
    if jpeg[0] != 0xFF || jpeg[1] != 0xD8 {
    ...
    loop {
        ...
        match m {
            0xDA => {
                ...
                let sp = sof_pos.ok_or_else(|| BlpError::new("jpeg_missing_sof0"))?;
                return Ok((sp, sos_hdr_end));
            }
            0xC0 => {
                // SOF0 baseline
                // segment starts 2 bytes before current length field
                let seg_start = i - 2;
                // advance past segment
                if i + 2 > jpeg.len() {
                    return Err(BlpError::new("jpeg_truncated_seg_len"));
                }
                let l = be_u16(&jpeg[i..i + 2])? as usize;
                i += 2;
                if i + (l - 2) > jpeg.len() {
                    return Err(BlpError::new("jpeg_truncated_sof0"));
                }
                i += l - 2;
                sof_pos = Some(seg_start); // include 0xFF 0xC0
            }
            0xC2 => return Err(BlpError::new("jpeg_progressive_unsupported")),
            0xC9 | 0xCA | 0xCB => return Err(BlpError::new("jpeg_arithmetic_unsupported")),
