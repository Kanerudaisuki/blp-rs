...
impl ImageBlp {
    pub fn encode_blp(&self, quality: u8) -> Result<Vec<u8>, BlpError> {
        let base: &RgbaImage = self
            .mipmaps
            .get(0)
            .and_then(|m| m.image.as_ref())
            .ok_or_else(|| BlpError::new("encode-blp.mip0-missing").with_arg("msg", "encode_blp: mip0 RGBA image is missing"))?;

        let (w0, h0) = (base.width(), base.height());
        if w0 == 0 || h0 == 0 {
            return Err(BlpError::new("encode-blp").with_arg("msg", "base image has zero dimension"));
        }
        ...
        for mip in &gen_mips {
            let img = mip
                .image
                .as_ref()
                .ok_or_else(|| BlpError::new("encode-blp.mip0-missing").with_arg("msg", "encode_blp: mip image missing"))?;
        ...
        if hdr_len < 2 || jpegs[0][0] != 0xFF || jpegs[0][1] != 0xD8 {
            return Err(BlpError::new("encode-blp").with_arg("msg", "encode_blp: not a JPEG (no SOI)"));
        }
...
// ----------------- helpers -----------------
fn build_mips_from_first(base: &RgbaImage, max_levels: usize) -> Result<Vec<Mipmap>, BlpError> {
    if max_levels == 0 {
        return Err(BlpError::new("build_mips_from_first: max_levels == 0"));
    }

    let (mut w, mut h) = base.dimensions();
    if w == 0 || h == 0 {
        return Err(BlpError::new("build_mips_from_first: base has zero dimension"));
    }
...
fn encode_argb_jpeg_full(img: &RgbaImage, w: u32, h: u32, quality: u8) -> Result<Vec<u8>, BlpError> {
